...
if __name__ == '__main__':
    ...
    if args["video"] and os.path.isfile(args["video"]):
        df = pd.DataFrame(
            columns=["Video_name", "frame_name", "face_name", "Anger", "Disgust", "Fear", "Happy", "Sad", "Surprise",
                     "Neutral"])
        ...
        for x in images:
            ...
            for y in temp_list:
                ...
                df = df.append(pd.Series(list_to_append,
                                         index=["Video_name", "frame_name", "face_name", "Anger", "Disgust", "Fear",
                                                "Happy", "Sad", "Surprise", "Neutral"]), ignore_index=True)
        name = "result" + ".csv"
        df.to_csv(name, sep=',')
